...
router.post(
  "/",
  requireAuth,
  async (req: AuthRequest & Request<{}, {}, CreateHabitBody>, res: Response) => {
    try {
...
router.put(
  "/:id",
  requireAuth,
  async (req: AuthRequest & Request<{ id: string }, {}, UpdateHabitBody>, res: Response) => {
    try {
      const { name, description, targetCount, color } = req.body;
...
router.post(
  "/:id/logs",
  requireAuth,
  async (req: AuthRequest & Request<{ id: string }, {}, HabitLogBody>, res: Response) => {
    try {
      ...
      let log = await prisma.habitLog.findFirst({ where: { habitId, date: today } });
