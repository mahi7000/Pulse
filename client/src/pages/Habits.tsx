...
const HABITS_PER_PAGE = 6; // 2 per row, 3 rows per page
...
export const HabitsPage = () => {
  ...
  const handleError = (error: unknown, defaultMessage: string) => {
    ...
    setError(error instanceof Error ? error.message : String(error) || defaultMessage);
  };
  ...
  const paginatedHabits = filteredHabits.slice((page - 1) * HABITS_PER_PAGE, page * HABITS_PER_PAGE);
  const totalPages = Math.ceil(filteredHabits.length / HABITS_PER_PAGE);

  const renderContent = () => {
    if (loading) return <p className="text-center py-8 text-gray-500">Loading your habits...</p>;
    if (error)
      return (
    ...
    if (filteredHabits.length === 0)
      return (
        <div className="text-center py-12">
          <p className="text-gray-500 mb-6 text-lg">You don't have any habits yet</p>
          <button
